...
TipoCelulaPilha *Topo; //Variável utilizada para manter o controle do estado atual da pilha
                  //Como a pilha é implementada em memória dinâmica,
                  //Topo deve ser um ponteiro para a célula no topo da Pilha

int ContadorPilha;  //Variável para contar o número de elementos armazenados na Pilha
...
void Pilha_Construtor()
{
  ...
  ContadorPilha = 0;  //Inicia a variável para indicar a quantidade de elementos armazenados na Pilha
}
...
void Pilha_Destrutor()
{
   ...
   TipoCelulaPilha *Temp;  //Ponteiro temporário utilizado para desalocar a memória
...
int Pilha_Tamanho()
{
    ...
    return ContadorPilha;   //É retornado o contador de elementos, manipulado nos métodos de Empilhar e Desempilhar
}
...
bool Pilha_Empilhar(TipoDadoPilha Elemento)
{

   TipoCelulaPilha *Nova;  //Ponteiro temporário utilizado para apontar para a nova célula a ser criada
   Nova = (TipoCelulaPilha *) malloc(sizeof(TipoCelulaPilha));
   ...
   else
   {
       ...
       ContadorPilha++;             //Como foi armazenado um novo Elemento, o contador deve ser incrementado
...
bool Pilha_Desempilhar(TipoDadoPilha &Elemento)
{
   ...
   if(!Pilha_Vazia())  //primeiro deve-se conferir se a Pilha não está vazia
   {
      ...
      TipoCelulaPilha *Temp = Topo;  //Cria um ponteiro temporário (Temp) e faz apontar para o mesmo endereço de Topo
      ...
      ContadorPilha--;          //Como foi removido um Elemento, o contador deve ser decrementado
